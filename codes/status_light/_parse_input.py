...
"处理数据的流程：输入==>(module_id,index,light)==>Code object"
...
def _parse(dic, raw_status, light_range):
    "从腾讯解析得来的setting json,参照 raw_status 的 modules_ids，检查dic里的module_id是否有效;u_count检查index是否过界，如有过界生成错误代码；并生成相应的灯光代码。(module_id,index,status) 这里index是倒过来的,机器顺序" 
    ...
    for data in datas:
        ...
        if u_id not in raw_status.keys():
            ...
            continue
            # 如果err
        u_count=raw_status[u_id].get("u_count")
        ...
        for u in statuses:
            ...
            if not result_u["err_code"]:
                lights_to_execute.append((u_id, u_count+1-index, status))
                continue
